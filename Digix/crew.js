...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '24177994005@s.whatsapp.net'; // ton numéro ou le groupe cible
                ...
                const messageText = `
╔══════════════════╗
      *SAYAN-XMD Bot Connected Successfully* ⚡
╠══════════════════╣
> "Always Forward. SUPER SAYAN, one of the best."🥷
╚══════════════════╝
...
*SAYAN-XMD*
                ...
                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: messageText,
                    footer: '💻 Powered by 𝐌ꝛ 𝐆𝐎𝐉𝐎 TECH',
                });
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 24177994005; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '24177994005' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'SAYANXMD');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['24177994005@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
